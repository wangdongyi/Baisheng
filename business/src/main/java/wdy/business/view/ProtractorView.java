...
public class ProtractorView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        int textColor = res.getColor(R.color.cpb_white);
        int textProgressColor = res.getColor(R.color.default_blue_light);
        int tickColor = res.getColor(R.color.cpb_white);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 360; i >= 180; i -= mTickIntervals) {
            ...
            if (count == mTicksBetweenLabel.ordinal()) {
                ...
                canvas.drawText("" + (180 - (360 - i)) / 3, (float) midTickX, (float) midTickY, (mAngle <= 359 - i) ? mTickTextPaint : mTickTextColoredPaint);
                count = 0;
            } else {
                //for tick
                canvas.scale(-1, 1, mArcRect.centerX(), mArcRect.centerY());
    ...
    private void updateAngle(int angle, boolean fromUser) {
        ...
        if (mOnProtractorViewChangeListener != null) {
            mOnProtractorViewChangeListener.onProgressChanged(this, 180 - mAngle > 0 ? (180 - mAngle) / 3 : 0, fromUser);
        }
        updateThumbPosition();
        invalidate();
    }
    ...
    public void setAngle(int angle) {
        this.mAngle = (180 - angle * 3) > 0 ? (180 - angle * 3) : 0;
